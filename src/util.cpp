...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "smartcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "smartcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Smartcoin will not work properly.");
