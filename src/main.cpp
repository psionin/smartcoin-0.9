...
uint256 hashGenesisBlock("0x142d2acd2480363e89fad37be56ff3b8993dd8da2be2525d75b02e202b0467a7");
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    // Litecoin: To limit dust spam, add nBaseFee for each output less than DUST_SOFT_LIMIT
    BOOST_FOREACH(const CTxOut& txout, vout)
...
static const int64 forkBlock4 = 385000; // February 2015 fork
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    uint64 TargetBlocksSpacingSeconds = nTargetSpacing;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (fTestNet) {
		...
		if (pindexLast->nHeight+1 >= 20) { DiffMode = 3; }
    ...
    switch (DiffMode) {
        case 1: return GetNextWorkRequired_V1(pindexLast, pblock);
        case 2: return KimotoGravityWell(pindexLast, pblock);
        case 3: return DigiShield(pindexLast, pblock);
		case 4: return DarkGravityWave(pindexLast, pblock);
        default: return GetNextWorkRequired_V1(pindexLast, pblock);
    }
}
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4381925df09d8d78d2d13214ed38fc82504b879563221ccf10b8dd8b52c2a6fc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1390694380; //epochtime
        ...
        assert(block.hashMerkleRoot == uint256("0xbc7cd78fd30e1ec47e66da50b6eedbaec8d95d01e68f58cb0b2c5e83e9cb8dd3"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SmartcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SmartcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SmartcoinMiner(CWallet *pwallet)
{
    printf("SmartcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("smartcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SmartcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SmartcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SmartcoinMiner, pwallet));
}
